    ...
    router.post("/user/register",function(req,res,next){
        ...
        pool.getConnection(function(err,conn){
            if(err){
                ...
                res.json(resData);//服务器端要求返回json 因此我们需要返回json数据格式
            }else {
                conn.query("select * from user where uname=?", [uname], function (err, result) {
                    if (err) {
                        ...
                        //说明没有注册过 那么就注册
                        conn.query("insert into user value(null,?,?,0)", [uname, pwd], function (err, result) {
                            conn.release();
                            if (err) {
                                resData.code = 0;
                                resData.message = "网络连接失败，请稍后重试";
        ...
        pool.getConnection(function(err,conn){
            if(err){
               ...
               conn.query("select * from user where uname=? and pwd=?",[uname,pwd],function(err,result){
                   ...
                   if(err){
                       resData.code=0;
                       resData.message="网络连接失败，稍后重试";
                       res.json(resData);
                   }else if(result.length<=0){
                       resData.code = 1;
                       resData.message = "用户名或者密码错误 请验证后再试";
                       res.json(resData);
                   }else {
                       resData.code = 2;
